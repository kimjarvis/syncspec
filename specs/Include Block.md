...
4. `IncludeBlockContext.state[key]`
5. `IncludeBlockContext.open_delimiter`
6. `block.suffix`
7. `IncludeBlockContext.close_delimiter`

Look the value in the dictionary `IncludeBlockContext.state[key]`.
- key shall be the value associated with the key "include" in the directive.
- If the key does not exist in the state dictionary then:
	- Return an object of type Error instead. 
	- Copy the `line_number` from Block.
	- Set `name` to be "test".
