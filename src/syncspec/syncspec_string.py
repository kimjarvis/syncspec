...
def make_syncspec_string(context: SyncspecStringContext) -> Callable[[Text], File]:
    # Verify context at initialization time
    if not context.open_delimiter or not context.close_delimiter:
        raise ValueError("open_delimiter and close_delimiter cannot be empty")
    if not Path(context.log).parent.exists():
        raise ValueError("log is not a valid file path")
    if not isinstance(context.G, nx.DiGraph):
        raise ValueError("G must be a valid nx.DiGraph object")
    if not isinstance(context.monad, dict):
        raise ValueError("monad must be a valid dictionary")
    if not Path(context.import_path).is_dir():
        raise ValueError("import_path must be a valid directory path")
    ...
    # Attach context for testing access
    context.combine_strings_context = csc
